...
// 350 movies ÷ 50 per batch = only 7 API calls!
//
// RETRY STRATEGY:
 ...
 */
async function extractAllEntities(pdfPath, totalMovies = 350, batchSize = 50) {
  // Upload PDF once
  ...
  console.log(`\n   📊 Pass 1: Extracting ${totalBatches} batches ...\n`);
  ...
  for (let i = 0; i < allBatches.length; i += CONCURRENCY) {
    ...
    if (i + CONCURRENCY < allBatches.length) {
      console.log("   ⏳ Waiting 4.5s for API rate limits...");
